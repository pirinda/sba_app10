 ...
 */
public class DViewDpsAccounts extends DGridPaneView implements ActionListener {

    private int mnSystemMoveClass;
    ...
    private JButton mjButtonCreatePayment;
    private JButton mjButtonClearPayment;
     ...
     */
    public DViewDpsAccounts(DGuiClient client, int type, int subtype, String title) {
        ...
        mnSystemMoveClass = mnGridSubtype == DModSysConsts.TS_DPS_CT_PUR ? DModSysConsts.FS_SYS_MOV_CL_MO : DModSysConsts.FS_SYS_MOV_CL_MI;
        ...
        mjButtonCreatePayment = DGridUtils.createButton(new ImageIcon(getClass().getResource("/sba/gui/img/cmd_pay_new.gif")), DUtilConsts.TXT_CREATE + " " + DFinConsts.getSysMovePaymentName(mnSystemMoveClass).toLowerCase(), this);
        getPanelCommandsSys(DGuiConsts.PANEL_CENTER).add(mjButtonCreatePayment);

        mjButtonClearPayment = DGridUtils.createButton(new ImageIcon(getClass().getResource("/sba/gui/img/cmd_pay_clr.gif")), DUtilConsts.TXT_CLEAR + " " + DFinConsts.getSysMovePaymentName(mnSystemMoveClass).toLowerCase(), this);
        getPanelCommandsSys(DGuiConsts.PANEL_CENTER).add(mjButtonClearPayment);
    }
    ...
    private void actionCreatePayment() {
        if (mjButtonCreatePayment.isEnabled()) {
            if (jtTable.getSelectedRowCount() != 1) {
                miClient.showMsgBoxInformation(DGridConsts.MSG_SELECT_ROW);
            }
            else {
                ...
                try {
                    if (gridRow.getRowType() != DGridConsts.ROW_TYPE_DATA) {
                        ...
                        params.setTypeKey(mnGridSubtype == DModSysConsts.TS_DPS_CT_PUR ? DModSysConsts.FS_SYS_MOV_TP_MO_VEN_PAY : DModSysConsts.FS_SYS_MOV_TP_MI_CUS_PAY);
                        params.getParamsMap().put(DGuiConsts.PARAM_DPS, gridRow.getRowPrimaryKey());
    ...
    private void actionClearPayment() {
        if (mjButtonClearPayment.isEnabled()) {
            if (jtTable.getSelectedRowCount() != 1) {
                miClient.showMsgBoxInformation(DGridConsts.MSG_SELECT_ROW);
            }
            else {
                ...
                try {
                    if (gridRow.getRowType() != DGridConsts.ROW_TYPE_DATA) {
                        ...
                        params = new DGuiParams();
                        params.getParamsMap().put(DGuiConsts.PARAM_DPS, gridRow.getRowPrimaryKey());
                        miClient.getSession().showForm(DModConsts.FX_CTM_DPS_CLR, DLibConsts.UNDEFINED, params);
                    }
                }
                catch (Exception e) {
                    DLibUtils.showException(this, e);
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT " +
                ...
                "HAVING NOT (f_bal = 0 AND f_bal_cy = 0) " +
                "ORDER BY " + orderBy;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() instanceof JButton) {
            ...
            else if (button == mjButtonCreatePayment) {
                actionCreatePayment();
            }
            else if (button == mjButtonClearPayment) {
                actionClearPayment();
            }
        }
    }

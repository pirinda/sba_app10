 ...
 */
public class DViewStockInventory extends DGridPaneView implements ActionListener {
     ...
     * @param type Type of stock. Constant defined in DModConsts (TX_STK_INV).
     * @param subtype Subtype of stock. Constants defined in DUtilConsts (PER_...).
     * @param title View title.
     */
    public DViewStockInventory(DGuiClient client, int type, int subtype, String title) {
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT " + select + " SUM(s.mov_in - s.mov_out) AS f_stk, " +
                ...
                "d.fk_dps_ct = " + DModSysConsts.TS_DPS_CT_PUR + " AND dr.fk_row_itm = s.id_itm AND dr.fk_adj_ct = " + DModSysConsts.TS_ADJ_CT_NA + " " +
                "ORDER BY d.dt DESC, d.ts_usr_upd DESC LIMIT 1), 0) AS f_lpp, " +
                "SUM(s.mov_in - s.mov_out) * COALESCE((SELECT dr.prc_unt " +
                "FROM t_dps AS d, t_dps_row AS dr " +
                "WHERE d.id_dps = dr.id_dps AND d.b_del = 0 AND dr.b_del = 0 AND " +
                "d.fk_dps_ct = " + DModSysConsts.TS_DPS_CT_PUR + " AND dr.fk_row_itm = s.id_itm AND dr.fk_adj_ct = " + DModSysConsts.TS_ADJ_CT_NA + " " +
                "ORDER BY d.dt DESC, d.ts_usr_upd DESC LIMIT 1), 0) AS f_inv " +
    ...
    public void createGridColumns() {
        ...
        columns[col] = new DGridColumnView(DGridConsts.COL_TYPE_DEC_AMT, "f_inv", "Inventario $ ML");

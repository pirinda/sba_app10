 ...
 */
public class DViewSpecialPrice extends DGridPaneView {

    public DViewSpecialPrice(DGuiClient client, String title) {
        super(client, DGridConsts.GRID_VIEW_TAB, DModConsts.M_SPE, DLibConsts.UNDEFINED, title);
    }
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT " +
                ...
                "FROM " + DModConsts.TablesMap.get(DModConsts.M_SPE) + " AS v " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CU_USR) + " AS ui ON " +
                "v.fk_usr_ins = ui.id_usr " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CU_USR) + " AS uu ON " +
                "v.fk_usr_upd = uu.id_usr " +
                (sql.length() == 0 ? "" : "WHERE " + sql) +
                "ORDER BY v.name, v.code, v.id_spe ";
    }

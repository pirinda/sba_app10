 ...
 */
public class DViewCustomerItemPriceType extends DGridPaneView {

    public DViewCustomerItemPriceType(DGuiClient client, String title) {
        super(client, DGridConsts.GRID_VIEW_TAB, DModConsts.M_CUS_ITM_PRC_TP, DLibConsts.UNDEFINED, title);
    ...
    public void prepareSqlQuery() {
        ...
        if ((Boolean) filter) {
            ...
            sqlBpr = " AND b.b_del = 0";
            sqlBprCt = " AND ct.b_del = 0";
        }
        ...
        msSql = "SELECT " +
                ...
                "FROM " + DModConsts.TablesMap.get(DModConsts.M_CUS_ITM_PRC_TP) + " AS v " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.MS_LNK_CUS_TP) + " AS vt ON " +
                "v.id_lnk_cus_tp = vt.id_lnk_cus_tp " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.MS_ITM_PRC_TP) + " AS ipt ON " +
                "v.fk_itm_prc_tp = ipt.id_itm_prc_tp " +
                ...
                "v.id_lnk_cus_tp = " + DModSysConsts.MS_LNK_CUS_TP_CUS_TP + " AND v.id_ref_cus = ct.id_cus_tp AND ct.b_del = 0 " +
                "WHERE (" +
                "(v.id_lnk_cus_tp = " + DModSysConsts.MS_LNK_CUS_TP_CUS + sqlBpr + ") OR " +
                "(v.id_lnk_cus_tp = " + DModSysConsts.MS_LNK_CUS_TP_CUS_TP + sqlBprCt + ")) " +
                (sql.length() == 0 ? "" : "AND " + sql) +
                "ORDER BY vt.name, vt.id_lnk_cus_tp, " + DDbConsts.FIELD_NAME + ", v.id_ref_cus ";
    }
    ...
    public void createGridColumns() {
        ...
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_TEXT_NAME_CAT_S, "ipt.name", DGridConsts.COL_TITLE_TYPE + " precio ítem");
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_DEC_PER_DISC, "v.dsc_per", "Descto. adic. %");

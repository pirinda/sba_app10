 ...
 */
public class DViewPromoPackage extends DGridPaneView {

    public DViewPromoPackage(DGuiClient client, String title) {
        super(client, DGridConsts.GRID_VIEW_TAB, DModConsts.M_PRM, DLibConsts.UNDEFINED, title);
    }
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT " +
                ...
                "FROM " + DModConsts.TablesMap.get(DModConsts.M_PRM) + " AS v " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.MS_ITM_PRC_TP) + " AS vt ON " +
                "v.fk_itm_prc_tp = vt.id_itm_prc_tp " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CU_USR) + " AS ui ON " +
                "v.fk_usr_ins = ui.id_usr " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CU_USR) + " AS uu ON " +
                "v.fk_usr_upd = uu.id_usr " +
                (sql.length() == 0 ? "" : "WHERE " + sql) +
                "ORDER BY v.name, v.code, v.id_prm ";
    }
    ...
    public void createGridColumns() {
        ...
        columns[col++] = new DGridColumnView(DGridConsts.COL_TYPE_TEXT_NAME_CAT_S, "vt.name", DGridConsts.COL_TITLE_TYPE + " precio ítem");

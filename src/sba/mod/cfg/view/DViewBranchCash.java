 ...
 */
public class DViewBranchCash extends DGridPaneView {

    public DViewBranchCash(DGuiClient client, String title) {
        super(client, DGridConsts.GRID_VIEW_TAB, DModConsts.CU_CSH, DLibConsts.UNDEFINED, title);
    }
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT " +
                ...
                "v.id_csh AS " + DDbConsts.FIELD_ID + "3, " +
                "v.code AS " + DDbConsts.FIELD_CODE + ", " +
                "v.name AS " + DDbConsts.FIELD_NAME + ", " +
                "vt.id_csh_tp AS " + DDbConsts.FIELD_TYPE_ID + "1, " +
                ...
                "FROM " + DModConsts.TablesMap.get(DModConsts.CU_CSH) + " AS v " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CS_CSH_TP) + " AS vt ON " +
                "v.fk_csh_tp = vt.id_csh_tp " +
                ...
                "v.fk_cur = c.id_cur " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.F_ABP_CSH) + " AS abp ON " +
                "v.fk_abp_csh = abp.id_abp_csh " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CU_USR) + " AS ui ON " +
                "v.fk_usr_ins = ui.id_usr " +
                "INNER JOIN " + DModConsts.TablesMap.get(DModConsts.CU_USR) + " AS uu ON " +
                "v.fk_usr_upd = uu.id_usr " +
                (sql.length() == 0 ? "" : "WHERE " + sql) +
                "ORDER BY v.name, v.code, v.id_bpr, v.id_bra, v.id_csh ";
    }
